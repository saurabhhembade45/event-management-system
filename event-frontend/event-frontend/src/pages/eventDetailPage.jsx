...
function EventDetailsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/payment/${event._id}`, {
        state: { participantDetails: formData }
      });
      setShowForm(false);
    } catch (error) {
      console.log(error);
  ...
  if (loading) {
    return <div className="dashboard"><h2>Loading event...</h2></div>;
  }

  if (!event) {
    return <div className="dashboard"><h2>Event not found</h2></div>;
  }
    ...
    <div className="dashboard">
      ...
      <div className="participate-container">
        <button
          ...
          style={{
            background: participated ? "#7c3aed" : "",
            cursor: participated ? "not-allowed" : "pointer",
            opacity: participated ? 0.8 : 1
          }}
        >
        ...
        >
          <div className="form-card" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              <input name="name" value={formData.name} placeholder="Full Name" required onChange={handleChange} />
              <input name="email" type="email" value={formData.email} placeholder="Email" required onChange={handleChange} />
              <input name="phone" value={formData.phone} placeholder="Phone Number" required onChange={handleChange} />
              <input name="college" value={formData.college} placeholder="College" onChange={handleChange} />
              ...
              <input name="emergencyContact" value={formData.emergencyContact} placeholder="Emergency Contact" onChange={handleChange} />
              <textarea name="requirements" value={formData.requirements} placeholder="Special Requirements" onChange={handleChange} />
